...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['receipts'])) {
    ...
    foreach ($_FILES['receipts']['tmp_name'] as $key => $tmpName) {
        ...
        $items = []; $total = 0; $stop_processing = false;

        foreach ($lines as $line) {
            if ($stop_processing) break;
            $text = str_replace([' ', '　'], '', $line['text']); // 彻底干掉空格

            // A. 合计检测（看到“合計”或“合计”就拿钱收工）
            if (preg_match('/(合計|合计|計)/u', $text) && preg_match('/(\d{1,3}(,\d{3})*)/', $text, $m)) {
                $total = (int)str_replace(',', '', $m[1]);
                $stop_processing = true; continue;
            }

            // B. 商品检测（必须包含 ¥ 符号）
            if (preg_match('/[¥￥]/u', $text)) {
                // 彻底排除你画红框的那些杂项
                if (preg_match('/(消費税|残高|支払|対象|番号|领收|軽|再発行)/u', $text)) continue;
                ...
                if (count($parts) >= 2) {
                    ...
                    if (preg_match('/(\d{1,3}(,\d{3})*)/', $parts[1], $m)) {
                        ...
                        if (!empty($name)) {
                             // 还原 ◎ 符号
                             $finalName = (strpos($line['text'], '◎') !== false ? '◎' : '') . $name;
...
<head>
    ...
    <style>
        ...
        .btn-main { background: #007aff; color: white; border: none; padding: 15px; width: 100%; border-radius: 8px; font-size: 18px; cursor: pointer; margin-top: 10px; }
        .card { border-left: 6px solid #34c759; background: #fafafa; padding: 15px; margin-top: 20px; border-radius: 4px; }
        .row { display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px dashed #ddd; }
        .total-val { text-align: right; color: #d32f2f; font-size: 28px; font-weight: bold; margin-top: 15px; }
...
<body>
    <div class="box">
        <h2 style="text-align:center;">📑 小票扫描准确版</h2>
        <form method="post" enctype="multipart/form-data">
            <input type="file" name="receipts[]" multiple><br>
