        ...
        if (move_uploaded_file($tmp, $path)) {
            ...
            if ($ocr && isset($ocr['analyzeResult']['readResults'])) {
                foreach ($ocr['analyzeResult']['readResults'][0]['lines'] as $line) {
                    ...
                    // A. 排除黑名单：绝对不是商品的行
                    if (preg_match('/責No|レジ|番号|領収|合計|小計|消費税|対象|再発行|残高/u', $text)) continue;

                    // B. 稳健匹配：商品名 + 任意字符 + 价格 (2-5位数字)
                    // 这个正则极其宽松，只要一行里同时有文字和数字，且数字在末尾附近就能抓到
                    if (preg_match('/^(.+?)\s+.*?([0-9,]{2,5})\s*[轻|軽|*|＊]?$/u', $text, $m)) {
                        $pName = trim($m[1]);
                        $price = (int)str_replace(',', '', $m[2]);

                        // C. 逻辑过滤：价格在合理范围内（50-5000日元）
...
<head>
    ...
    <style>
        ...
        .btn-group { margin-top: 25px; background: #eef3f8; padding: 15px; border-radius: 8px; }
...
<body>
<div class="card">
    <h2>🏪 FamilyMart 识别系统</h2>
    <form method="post" enctype="multipart/form-data">
        <input type="file" name="images[]" multiple required>
        <br><br>
        <button type="submit" style="padding: 10px 20px; cursor: pointer; background: #28a745; color: white; border: none; border-radius: 5px; font-size: 1em;">上传并解析收据</button>
    </form>
    ...
    <?php if ($displayItems): ?>
        <div style="margin-top: 25px;">
            ...
            <div class="btn-group">
                <strong>📥 数据导出：</strong><br><br>
                <a href="result.csv" class="btn" download>下载 CSV</a>
                <a href="ocr.log" class="btn" target="_blank">查看 OCR 日志</a>
            </div>
        </div>
    <?php elseif ($_SERVER['REQUEST_METHOD'] == 'POST'): ?>
        <p style="color:red; margin-top:20px;">未能提取商品，请检查 ocr.log。</p>
    <?php endif; ?>
</div>
</body>
</html>
